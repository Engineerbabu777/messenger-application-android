...
export default function HomeScreen() {
	...
	useLayoutEffect(() => {
		navigation.setOptions({
			headerTitle: "",
			headerLeft: () => (<Text style={{fontSize:20,fontWeight:'bold',}}>Babu Chat</Text>),
			headerRight: () => (
    ...
    useEffect(() => {
       const fetchUsers = async() => {
        ...
        setUserId(userId);
        // MAKE A REQUEST!
        const response = await axios.get('http://192.168.103.155:8080/users/'+userId);
	...
	return (
		<View>
            <View>
                {users.map((user,ind) => (<View key={ind} style={{padding:10}}>
                    <User user={user}/>
                </View>))}
            </View>
		</View>
